...
/**
 * Scenes whose copy sits in a band across the top of the viewport, with the
 * interface directly beneath it. Their copy travels up through the interface
 * area before the section pins, so the interface has to wait for the hold —
 * on every layout, not just the stacked one.
 *
 * The side-by-side scenes (SEO, GEO) put copy and interface in opposite
 * columns, so they have nothing to wait for on wide screens.
 */
export const usesTopBand = (sceneIndex) => sceneIndex >= 3 && sceneIndex <= 7;

/**
 * Windows where a full-screen statement owns the frame. While one is on
 * screen the scene navigator steps aside, so cinematic type never has to be
 * narrowed to avoid colliding with fixed chrome.
 *
 * Values are scene-local progress (0 -> 1) and are the only place these
 * boundaries are declared; the scenes own the detail beats inside them.
 */
export const statementWindows = [
  { scene: 2, from: 0.42, to: 1.02 }, // "Search has changed" -> "SEO + GEO"
  { scene: 7, from: 0.36, to: 1.02 }, // "Research. Create. Rank. Improve. Repeat."
  { scene: 8, from: 0.4, to: 1.02 }, // "Your next customer is searching."
];
